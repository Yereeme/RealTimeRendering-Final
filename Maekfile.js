...
const main_objs = [
	...
	maek.CPP('tinyobjloarder_impl.cpp'),
	maek.CPP('frame_time_logger.cpp'),
	maek.CPP('stb_image_impl.cpp'),
	maek.CPP('stb_image_write_impl.cpp'),
	maek.CPP('RTG.cpp'),
...
const objects_vert = maek.GLSLC('objects.vert');
...
const main_exe = maek.LINK([...main_objs, ...prebuilt_objs], 'bin/viewer');

//A2 utility: cube-lambertian (CLI tool)
// ---- A2 utility: cube-lambertian ----
const cube_lambertian_objs = [
	maek.CPP('cube-lambertian.cpp'),
	// reuse the ones already created earlier:
	'objs/stb_image_impl' + maek.DEFAULT_OPTIONS.objSuffix,
	'objs/stb_image_write_impl' + maek.DEFAULT_OPTIONS.objSuffix,
];

const cube_lambertian_exe = maek.LINK([...cube_lambertian_objs, ...prebuilt_objs], 'bin/cube-lambertian');

const brdf_lut_objs = [
	maek.CPP('brdf-lut.cpp'),
	// reuse the ones already created earlier:
	'objs/stb_image_impl' + maek.DEFAULT_OPTIONS.objSuffix,
	'objs/stb_image_write_impl' + maek.DEFAULT_OPTIONS.objSuffix,
];

const brdf_lut_exe = maek.LINK([...brdf_lut_objs, ...prebuilt_objs], 'bin/brdf-lut');


//default targets:
maek.TARGETS = [main_exe, cube_lambertian_exe, brdf_lut_exe];
