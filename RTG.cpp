...
void RTG::Configuration::parse(int argc, char **argv) {
	for (int argi = 1; argi < argc; ++argi) {
		...
		if (arg == "--debug") {
			...
			if (argi + 1 >= argc) throw std::runtime_error("--exposure requires a parameter (a number).");
			argi += 1;

			// let std::stof handle stuff like -3, 0, 1.5, etc
			try {
				exposure = std::stof(argv[argi]);
			}
			catch (...) {
				throw std::runtime_error(std::string("--exposure must be a number, got '") + argv[argi] + "'.");
			}
		}
		else if (arg == "--exposure") {
    if (argi + 1 >= argc) throw std::runtime_error("--exposure requires a parameter (a number).");
    ...
    try {
        ...
        throw std::runtime_error(std::string("--exposure must be a number, got '") + argv[argi] + "'.");
			...
			if (argi + 1 >= argc) throw std::runtime_error("--tone-map requires a parameter (linear or reinhard).");
			argi += 1;

			tone_map = argv[argi];

			//  trying to be careful about typos 
			if (tone_map != "linear" && tone_map != "reinhard") {
				throw std::runtime_error("--tone-map must be 'linear' or 'reinhard', got '" + tone_map + "'.");
			}
		}
...
void RTG::Configuration::usage(std::function< void(const char *, const char *) > const &callback) {
	...
	callback("--exposure <E>", "Multiply radiance by 2^E before tone mapping. Default: 0.");
callback("--tone-map linear|reinhard", "Select tone mapping operator. Default: linear.");
			...
			if (configuration.headless) {
				...
				if (configuration.debug) {
					...
					for (size_t i = 0; i < present_modes.size(); ++i) {
						std::cout
							<< "[" << i << "] "
							<< string_VkPresentModeKHR(present_modes[i])
							<< "\n";
					}
...
void RTG::recreate_swapchain() {
	//clean up swapchain if it already exits
	...
	if (configuration.headless) {
		...
		for (uint32_t i = 0; i < requested_count; ++i) {
			...
			h.image = helpers.create_image(
				swapchain_extent, // width/height of the image
				surface_format.format, // pixel format (e.g., BGRA8)
				VK_IMAGE_TILING_OPTIMAL,     // GPU-friendly layout in VRAM
				VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT,  // render into it + copy out of it
				VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT // allocate in fast GPU memory (VRAM)
			);
			...
			{ //create fence to signal when image is done being "presented" (copied to host memory):
				VkFenceCreateInfo create_info{ //CPU–GPU synchronization object.
					.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO,
		...
		{//make the swapchain
			VkSwapchainCreateInfoKHR create_info{
				...
				.clipped = VK_TRUE,
				.oldSwapchain = VK_NULL_HANDLE //NOTE: could be more efficient by passing old swapchain handle 
				//here instead of destroying it
			};
...
void RTG::destroy_swapchain() {
	...
	if (configuration.headless) {
		//destroy headless_swapchain images ( we loop through each image, destroying its resources; 
...
void RTG::run(Application& application) {
	...
	while (configuration.headless || !glfwWindowShouldClose(window)) {
		...
		{//acquire a workspace (this workspace_available fence is exactly the one our
			//render function passes to vkQueueSubmit to signal when the rendering work is done.
